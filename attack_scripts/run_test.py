...
def load_process_traces():
    ...
    return num_traces*len(setting_mod.trace_list)
...
total_traces = load_process_traces()
print("Starting text loading...")
load_text()
print("Starting analysis...")

def run_engine(target_byte, total_traces):
    ...
    dict_output_method = DictOutputMethod(cpa_engines[0],filename=(setting_mod.prefix_batch + "_results_" + str(target_byte) + ".pickle"))

    output_steps = [ i for i in range(0, total_traces, setting_mod.report_interval)]

    container_textout = NpyContainer(setting_mod.prefix_batch + "_traces.npy", setting_mod.prefix_batch + "_texts.npy")
...
for p in byte_list:    
    p = Process(target=run_engine, args=(p,total_traces))
    p.start()
    p_list.append(p)
